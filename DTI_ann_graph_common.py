...
data_dir = '../../Documents/LinkPredI_data/'

with open(data_dir+'drugs2tars_stitch.dict','r') as f:
	drugs_targets = json.load(f)


data = pd.read_csv(data_dir+'embeddings_LinkPredI_has_target_free.txt', header = None, skiprows = 1, sep = ' ')
embds_data = data.values
embds_dict = dict(zip(embds_data[:,0],embds_data[:,1:]))

data = pd.read_csv(data_dir+'mapping_LinkPredI.txt', header = None, sep = '\t')
data = data.values
